...
import com.imooc.myo2o.exception.ProductCategoryOperationExecution;
...
public class ProductCategoryServiceImpl implements ProductCategoryService {
	...
	public ProductCategoryExecution deleteProductCategory(
		...
		//将此商品类别下的商品的类别Id置为空,解除product与productcatregoryId的关联
		try {
			int effectedNum = productDao
					.updateProductCategoryToNull(productCategoryId);
			if (effectedNum < 0) {
				throw new ProductCategoryOperationExecution("商品类别更新失败");
			}
		} catch (Exception e) {
			throw new ProductCategoryOperationExecution("deleteProductCategory error: "
					+ e.getMessage());
		}
		...
		try {
			...
			if (effectedNum <= 0) {
				throw new ProductCategoryOperationExecution("店铺类别删除失败");
			} else {
				return new ProductCategoryExecution(
						ProductCategoryStateEnum.SUCCESS);
			}
